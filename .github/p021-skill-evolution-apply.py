...
def replace_one(path: str, old: str, new: str):
    ...
    if count != 1:
        raise SystemExit(f"{path}: expected exactly one literal match, found {count}: {old[:160]!r}")
    p.write_text(text.replace(old, new, 1))


def regex_one(path: str, pattern: str, replacement: str):
    p = Path(path)
    text = p.read_text()
    new_text, count = re.subn(pattern, replacement, text, count=1, flags=re.S)
    if count != 1:
        raise SystemExit(f"{path}: expected exactly one regex match, found {count}: {pattern[:160]!r}")
    p.write_text(new_text)


# ── Schema v20: durable skill evolution authority ─────────────────────────────
replace_one(
    "src/state/schema.ts",
    "export const SCHEMA_VERSION = 19;",
    "export const SCHEMA_VERSION = 20;",
)
schema = Path("src/state/schema.ts")
schema_text = schema.read_text()
marker = "export const MIGRATION_V19_CAPABILITY_LIFECYCLE = `"
if marker not in schema_text:
    raise SystemExit("schema.ts: V19 migration marker missing")
if "MIGRATION_V20_SKILL_EVOLUTION" not in schema_text:
    schema_text += r'''
...
replace_one(
    ...
    {
      version: 20,
      apply: () => db.exec(MIGRATION_V20_SKILL_EVOLUTION),
    },
  ];''',
)

# ── Tool context gets the existing CapabilityRegistry authority ───────────────
replace_one(
    "src/types.ts",
...
# ── CapabilityRegistry: versioned skill contract projection ──────────────────
...
regex_one(
    path,
    r'''  ingestSkills\(skills: Array<\{ name: string; description\?: string; enabled\?: boolean \}>\): void \{.*?\n  \}\n\}''',
...
Path("src/skills/evolution.ts").write_text(r'''import { createHash } from "node:crypto";
...
export function isSkillEvolutionManaged(db: Database, skillName: string): boolean {
...
# ── Lifecycle tools ────────────────────────────────────────────────────────────
Path("src/skills/evolution-tools.ts").write_text(r'''import type { AbosTool, SkillSource, ToolContext } from "../types.js";
...
# ── Loader runtime truth ───────────────────────────────────────────────────────
path = "src/skills/loader.ts"
replace_one(path, 'import { expandHomePath } from "../platform/home.js";\n', 'import { expandHomePath } from "../platform/home.js";\nimport { isSkillEvolutionManaged } from "./evolution.js";\n')
replace_one(path, '''        // Check if already in DB and preserve enabled state
        ...
        db.upsertSkill(skill);''', '''        if (!isSkillEvolutionManaged(db.raw, skill.name)) {
          const existing = db.getSkillByName(skill.name);
          if (existing) {
            skill.enabled = existing.enabled;
            skill.installedAt = existing.installedAt;
...
replace_one(path, "function checkRequirements(skill: Skill): boolean {", "export function checkSkillRequirements(skill: Skill): boolean {")
Path(path).write_text(Path(path).read_text().replace("checkRequirements(skill)", "checkSkillRequirements(skill)"))

# ── Legacy registry cannot bypass managed authority ───────────────────────────
path = "src/skills/registry.ts"
replace_one(path, 'import { expandHomePath } from "../platform/home.js";\n', 'import { expandHomePath } from "../platform/home.js";\nimport { isSkillEvolutionManaged } from "./evolution.js";\n')
replace_one(path, '''  // Validate inputs to prevent injection
  if (!SKILL_NAME_RE.test(name)) {''', '''  if (isSkillEvolutionManaged(db.raw, name)) {
    throw new Error(`Skill ${name} is managed by Skill Evolution; create a new version instead of overwriting it.`);
  }
  // Validate inputs to prevent injection
  if (!SKILL_NAME_RE.test(name)) {''')
replace_one(path, '''export async function installSkillFromUrl(
  url: string,
  name: string,
  skillsDir: string,
  db: AbosDatabase,
  ...
  // Validate inputs to prevent injection''', '''export async function installSkillFromUrl(
  url: string,
  name: string,
  skillsDir: string,
  db: AbosDatabase,
  ...
  // Validate inputs to prevent injection''')
replace_one(path, '''export async function createSkill(
  name: string,
  description: string,
  instructions: string,
  skillsDir: string,
  ...
  // Validate name to prevent path traversal/injection''', '''export async function createSkill(
  name: string,
  description: string,
  instructions: string,
  skillsDir: string,
...
replace_one(path, '''export async function removeSkill(
  name: string,
  db: AbosDatabase,
  _conway: ConwayClient,
  skillsDir: string,
  ...
  // Validate name to prevent path traversal/injection''', '''export async function removeSkill(
  name: string,
  db: AbosDatabase,
  _conway: ConwayClient,
  skillsDir: string,
...
replace_one(path, '''    ...installedTools,
    ...environmentTools,
  ]);
  capabilityRegistry.ingestSkills(skills ?? []);''', '''    ...installedTools,
    ...environmentTools,
    ...skillEvolutionTools,
  ]);
...
replace_one(path, '''      ...installedTools,
      ...environmentTools,
    ].map((tool) => tool.name),''', '''      ...installedTools,
      ...environmentTools,
      ...skillEvolutionTools,
    ].map((tool) => tool.name),''')
replace_one(path, '''    ...installedTools,
...
replace_one(path, '''    inference,
    social,
  };''', '''    inference,
    social,
    capabilityRegistry,
  };''')
replace_one(path, '''      const systemPrompt = buildSystemPrompt({
        identity,
        config,
        financial,
...
Path("src/__tests__/skills-evolution.test.ts").write_text(r'''import { mkdtempSync, rmSync, mkdirSync, writeFileSync } from "node:fs";
