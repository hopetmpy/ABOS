...
''',
  ...
  private terminateProcessTree(entry: ManagedProcessEntry, mode: "cancel" | "kill"): void {
    ...
    if (process.platform === "win32") {
      try {
        // Windows does not provide POSIX SIGTERM/SIGKILL process-group semantics
        // through Node. taskkill /T is the host-native tree primitive; /F is
        // required so cancellation cannot report the wrapper closed while
        // descendants continue executing outside the managed handle.
    ...
    try {
      // start() launches the shell detached so its PID is also the process-group
      // id. Signal the whole group instead of only sh/bash -lc.
...
''',
    '''      description: "Request cancellation of a managed local process tree and report observed state. POSIX uses SIGTERM on the owned process group; Windows uses host-native task-tree termination and is not represented as POSIX-graceful semantics.",
''',
)
