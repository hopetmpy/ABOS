...
cleanup() {
  ...
  if [[ -n "$ABOS_PID" ]] && kill -0 "$ABOS_PID" 2>/dev/null; then
    kill "$ABOS_PID" 2>/dev/null || true
    wait "$ABOS_PID" 2>/dev/null || true
  fi
  summarize
  exit 0
}
...
get_rss_kb() {
  if [[ -n "$ABOS_PID" ]] && kill -0 "$ABOS_PID" 2>/dev/null; then
    ps -o rss= -p "$ABOS_PID" 2>/dev/null | tr -d ' ' || echo "0"
  else
    echo "0"
  fi
}
...
if ! kill -0 "$ABOS_PID" 2>/dev/null; then
  log "ERROR: ABOS process failed to start"
  exit 1
fi
...
while [[ $(date +%s) -lt $END_TIME ]]; do
  if ! kill -0 "$ABOS_PID" 2>/dev/null; then
    log "ERROR: ABOS process died unexpectedly"
    summarize
    exit 1
  fi
...
log "Soak test duration complete, stopping abos..."
kill "$ABOS_PID" 2>/dev/null || true
wait "$ABOS_PID" 2>/dev/null || true
