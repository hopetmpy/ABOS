...
function makeTmpDbPath(): string {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "abos-data-layer-test-"));
...
describe("Agent Card Validation", () => {
  ...
  it("rejects missing name", () => {
    expect(validateAgentCard({ type: "abos" })).toBeNull();
  });
  ...
  it("rejects empty name", () => {
    expect(validateAgentCard({ name: "", type: "abos" })).toBeNull();
  });
  ...
  it("rejects non-string address", () => {
    expect(validateAgentCard({ name: "TestAgent", type: "abos", address: 123 })).toBeNull();
  });

  it("accepts card without optional fields", () => {
    const card = validateAgentCard({ name: "TestAgent", type: "abos" });
    expect(card).not.toBeNull();
  });
});
