...
export async function connectCodex(config: AbosConfig): Promise<number> {
  ...
  console.log(
    chalk.dim(
      "  Complete authorization in your browser. ABOS never asks for your ChatGPT password or OAuth token.",
    ),
  );
...
export async function runCodexLogin(): Promise<void> {
  ...
  // Resolve through the same open adapter registry used by Connect AI so the
  // direct convenience command does not create a second activation path.
  ...
  const adapter = createBuiltinAiConnectionAdapterRegistry().get("codex");
  if (adapter) {
...
export async function disconnectCodex(config: AbosConfig): Promise<void> {
  ...
  const connections = { ...(config.aiConnection?.connections || {}) };
  for (const [id, profile] of Object.entries(connections)) {
    if (profile.provider === "codex") delete connections[id];
  }
  config.aiConnection = {
    ...(config.aiConnection || {}),
    connections,
...
export async function runCodexModels(): Promise<void> {
  ...
  for (const model of snapshot.models) {
    ...
    console.log(
      `  ${chalk.white(model.model)}  ${chalk.dim(model.displayName)}${marker}`,
    );
